...
pub enum Error {
    ...
    /// The requested SIMD backend is not available on this platform or CPU.
    UnsupportedBackend,
}
...
impl std::fmt::Display for Error {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Error::UnsupportedBackend => write!(f, "unsupported SIMD backend"),
        }
    }
}
...
impl OilScale {
    ...
    /// Ingest one input scanline using the scalar (non-SIMD) backend.
    ///
    /// See [`push_scanline`](Self::push_scanline) for details.
    pub fn push_scanline_scalar(&mut self, input: &[u8]) -> Result<(), Error> {
        if self.slots() == 0 {
            return Err(Error::InvalidArgument);
        }
        if self.is_upscale {
    ...
    /// Ingest one input scanline using the SSE2 backend.
    ///
    /// Returns `Err(Error::UnsupportedBackend)` on non-x86_64 platforms.
    /// See [`push_scanline`](Self::push_scanline) for details.
    pub fn push_scanline_sse2(&mut self, input: &[u8]) -> Result<(), Error> {
        #[cfg(target_arch = "x86_64")]
        {
            if self.slots() == 0 {
                return Err(Error::InvalidArgument);
    ...
    /// Ingest one input scanline using the AVX2 backend.
    ///
    /// Returns `Err(Error::UnsupportedBackend)` if the current CPU does not
    /// support AVX2+FMA or on non-x86_64 platforms.
    /// See [`push_scanline`](Self::push_scanline) for details.
    pub fn push_scanline_avx2(&mut self, input: &[u8]) -> Result<(), Error> {
        #[cfg(target_arch = "x86_64")]
        {
    ...
    /// Ingest one input scanline using the NEON backend.
    ///
    /// Returns `Err(Error::UnsupportedBackend)` on non-aarch64 platforms.
    /// See [`push_scanline`](Self::push_scanline) for details.
    pub fn push_scanline_neon(&mut self, input: &[u8]) -> Result<(), Error> {
        #[cfg(target_arch = "aarch64")]
        {
            if self.slots() == 0 {
                return Err(Error::InvalidArgument);
    ...
    /// Produce the next scaled output scanline using the scalar (non-SIMD) backend.
    ///
    /// See [`read_scanline`](Self::read_scanline) for details.
    pub fn read_scanline_scalar(&mut self, output: &mut [u8]) -> Result<(), Error> {
        if self.slots() != 0 {
            return Err(Error::InvalidArgument);
        }
        if self.is_upscale {
    ...
    /// Produce the next scaled output scanline using the SSE2 backend.
    ///
    /// Returns `Err(Error::UnsupportedBackend)` on non-x86_64 platforms.
    /// See [`read_scanline`](Self::read_scanline) for details.
    pub fn read_scanline_sse2(&mut self, output: &mut [u8]) -> Result<(), Error> {
        #[cfg(target_arch = "x86_64")]
        {
            if self.slots() != 0 {
                return Err(Error::InvalidArgument);
    ...
    /// Produce the next scaled output scanline using the AVX2 backend.
    ///
    /// Returns `Err(Error::UnsupportedBackend)` if the current CPU does not
    /// support AVX2+FMA or on non-x86_64 platforms.
    /// See [`read_scanline`](Self::read_scanline) for details.
    pub fn read_scanline_avx2(&mut self, output: &mut [u8]) -> Result<(), Error> {
        #[cfg(target_arch = "x86_64")]
        {
            if !(is_x86_feature_detected!("avx2") && is_x86_feature_detected!("fma")) {
                return Err(Error::UnsupportedBackend);
    ...
    /// Produce the next scaled output scanline using the NEON backend.
    ///
    /// Returns `Err(Error::UnsupportedBackend)` on non-aarch64 platforms.
    /// See [`read_scanline`](Self::read_scanline) for details.
    pub fn read_scanline_neon(&mut self, output: &mut [u8]) -> Result<(), Error> {
        #[cfg(target_arch = "aarch64")]
        {
            if self.slots() != 0 {
                return Err(Error::InvalidArgument);
...
impl OilScale {
    ...
    #[allow(dead_code)] // used by auto-dispatch when force-scalar is not set
    ...
    unsafe fn down_scale_in_sse2_only(&mut self, input: &[u8]) {
        let coeffs_y_start = self.in_pos as usize * 4;
        let coeffs_y = [
            self.coeffs_y[coeffs_y_start],
            self.coeffs_y[coeffs_y_start + 1],
        ...
        match self.cs {
            ...
            ColorSpace::G => sse2::scale_down_g(
                input, &mut self.sums_y, self.out_width, &self.coeffs_x, &self.borders_x, &coeffs_y,
            ),
            ...
            ColorSpace::RgbaNoGamma => sse2::scale_down_rgba_nogamma(
                input, &mut self.sums_y, self.out_width, &self.coeffs_x, &self.borders_x, &coeffs_y, self.sums_y_tap,
            ),
            ColorSpace::RgbxNoGamma => sse2::scale_down_rgbx_nogamma(
                input, &mut self.sums_y, self.out_width, &self.coeffs_x, &self.borders_x, &coeffs_y, self.sums_y_tap,
            ),
        }
    }
    ...
    unsafe fn down_scale_in_avx2_prefer(&mut self, input: &[u8]) {
        ...
        match self.cs {
            ColorSpace::RGB => avx2::scale_down_rgb(
                input, &mut self.sums_y, self.out_width, &self.coeffs_x, &self.borders_x, &coeffs_y,
                srgb::tables().s2l.as_ptr(),
            ),
            ...
            ColorSpace::RgbNoGamma => avx2::scale_down_rgb(
                input, &mut self.sums_y, self.out_width, &self.coeffs_x, &self.borders_x, &coeffs_y,
                srgb::tables().i2f.as_ptr(),
            ),
    ...
    unsafe fn down_scale_out_sse2_only(&mut self, output: &mut [u8]) {
        let cmp = self.cs.components();
        let sl_len = self.out_width as usize * cmp;
        let tap = self.sums_y_tap;
        ...
        match self.cs {
            ...
            ColorSpace::G | ColorSpace::CMYK | ColorSpace::RgbNoGamma => sse2::yscale_out_g(&mut self.sums_y, sl_len, output),
            ColorSpace::GA => sse2::yscale_out_ga(&mut self.sums_y, self.out_width, output),
            ColorSpace::RgbaNoGamma => sse2::yscale_out_rgba_nogamma(&mut self.sums_y, self.out_width, output, tap),
            ColorSpace::RgbxNoGamma => sse2::yscale_out_rgbx_nogamma(&mut self.sums_y, self.out_width, output, tap),
        }
    }

    unsafe fn down_scale_out_avx2_prefer(&mut self, output: &mut [u8]) {
        let cmp = self.cs.components();
        let sl_len = self.out_width as usize * cmp;
        let tap = self.sums_y_tap;
        ...
        match self.cs {
            ...
            ColorSpace::G | ColorSpace::CMYK | ColorSpace::RgbNoGamma => avx2::yscale_out_g(&mut self.sums_y, sl_len, output),
            ColorSpace::GA => sse2::yscale_out_ga(&mut self.sums_y, self.out_width, output),
            ColorSpace::RgbaNoGamma => avx2::yscale_out_rgba_nogamma(&mut self.sums_y, self.out_width, output, tap),
            ColorSpace::RgbxNoGamma => avx2::yscale_out_rgbx_nogamma(&mut self.sums_y, self.out_width, output, tap),
        }
    }
}
